/**
 * SeleccionPlazaPage.js — v6
 * FIXES:
 *  1. "Sin convocatoria seleccionada" desaparece cuando se detecta postulación activa
 *     (se guarda el convocatoria_id dinámicamente en estado local)
 *  2. Modo edición automático: si el docente ya tiene postulación activa,
 *     puede editar y guardar directamente sin necesidad del botón "Editar"
 *     desde la página de convocatorias.
 */
...
const SeleccionPlazaPage = () => {
    ...
    // convocatoriaIdParam: lo que viene de URL/state (puede ser null si entra por menú)
    ...
    // ── Estado dinámico de IDs (se actualizan cuando se detecta postulación activa) ──
    ...
    //         para que desaparezca la alerta "Sin convocatoria seleccionada"
        ...
        const inicializar = async () => {
            try {
                ...
                if (!convId) {
                    ...
                    if (items.length === 0) return  // no tiene postulación activa → wizard vacío OK
                    convId = items[0].convocatoria_id

                    // FIX 1: Guardar el postulacion_id si ya lo tenemos aquí
                ...
                if (res.data && res.data.id) {
                    // FIX 1: Actualizar convocatoriaId dinámico → desaparece la alerta
                    setConvocatoriaId(String(convId))

                    // FIX 2: Si ya tiene plaza seleccionada → activar modo edición automático
    ...
    const confirmarPostulacion = async () => {
        ...
        try {
            ...
            if (modoEditar && postulacionId) {
                // PATCH — modificar plaza existente
                                ...
                                <div>
                                    ...
                                    <div className='row mb-5'>
                                        ...
                                        <div className='col-md-4 mb-3'>
                                            <div
                                                ...
                                                onClick={() => setSeleccion(p => ({ ...p, caracteristica: null }))}
                                            >
                                                <div
                                        ...
                                        {caracteristicas
                                            ...
                                            .map((caract) => {
                                                const esBilingue = caract.nombre?.toUpperCase().includes('BILING')
                                                const esConvenio = caract.nombre?.toUpperCase().includes('CONVEN')
                                                const icon = esBilingue ? 'fa-language' : esConvenio ? 'fa-handshake' : 'fa-star'
                                                const color = esBilingue ? '#8950FC' : esConvenio ? '#FFA800' : '#3699FF'
                                                const bg = esBilingue ? '#EEE5FF' : esConvenio ? '#FFF4DE' : '#EEF6FF'
                                                    ...
                                                    <div key={caract.id} className='col-md-4 mb-3'>
                                                        <div
                                                            ...
                                                            style={{
                                                                cursor: 'pointer',
                                                                transition: 'all 0.2s',
                                                                borderColor: seleccionado ? color : '#EBEDF3',
                                                                background: seleccionado ? bg : '#fff',
                                                                boxShadow: seleccionado ? `0 0 0 2px ${color}33` : 'none',
                                                            }}
                                    ...
                                    {tipoCaracteristica() === 'CONVENIO' && (
                                        <div className='rounded p-5 mb-5' style={{ background: '#FFF4DE', borderLeft: '4px solid #FFA800' }}>
                                            <div className='font-weight-bolder mb-3' style={{ color: '#FFA800' }}>
                                                ...
                                                >
                                                    ...
                                                    {convenioArchivo ? (
                                                        <div className='d-flex align-items-center justify-content-center'>
                                                            <i className='fas fa-file-alt mr-2' style={{ color: '#FFA800', fontSize: 20 }} />
                                                        ...
                                                        <>
                                                            <i className='fas fa-cloud-upload-alt text-muted mb-2' style={{ fontSize: 28 }} />
                                                            <div className='text-muted font-size-sm'>
                                    ...
                                    {/* Aviso si el botón está deshabilitado por falta de convocatoria */}
                                    {!convocatoriaId && (
                    ...
                    <div className='card card-custom'>
                        ...
                        <div className='card-body p-6'>
                            ...
                            <div className='d-flex align-items-center mb-6'>
                                <div
                                    className='d-flex align-items-center justify-content-center rounded mr-3'
                                    style={{ width: 36, height: 36, background: '#E8FFF3', flexShrink: 0 }}
                                >
                                    <i className='fas fa-university' style={{ color: '#1BC5BD', fontSize: 16 }} />
                                </div>
                                <div>
                            ...
                            {/* Badge modo edición en panel lateral */}
                            {modoEditar && (
                            ...
                            {/* Alerta sin convocatoria en panel lateral — solo si no hay ID */}
                            {!convocatoriaId && (
                                <div className='rounded p-3 mt-5'
                                    style={{ background: '#FFF4DE', borderLeft: '3px solid #FFA800' }}>
                                    <div className='font-weight-bold font-size-xs mb-1' style={{ color: '#FFA800' }}>
