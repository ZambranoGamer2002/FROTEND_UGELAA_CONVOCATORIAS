...
export function AsideMenuList({layoutProps}) {
  ...
  // Extraer token de cualquier formato que use Metronic en Redux
  const getToken = () => {
    if (!auth) return null
    // Intentar distintas claves que usa Metronic
  ...
  useEffect(() => {
    // También intentar leer de localStorage directamente como fallback
        ...
        {roleInfo && (
          <li className='menu-section mt-4'>
            <div className='px-5 pb-2 w-100'>
              <span
                className={`badge badge-pill w-100 py-2 ${
                  roleInfo.nivel === 1 ? 'badge-danger'  :
